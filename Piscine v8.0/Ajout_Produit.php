...
if($type === "Musiques") {
    ...
    /////////////Pour la Video Musique
    //"Images/Video/" = subdirectory for images in www directory
    $target_dir = "Images/Video/";
    $target_file = $target_dir . basename($_FILES["fileToUpload_Video_Musique"]["name"]);
    $uploadOk = 1;
    //file extension in lower case
    $VideoFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Vérifier si le fichier image est une image réelle ou une image fausse
    if(isset($_POST["SignIn"])) {
        $check = getimagesize($_FILES["fileToUpload_Video_Musique"]["tmp_name"]);
        if($check !== false) {
            echo "Le fichier est une video - " . $check["mime"] . ".";
            $uploadOk = 1;
        } else {
            //echo "Le fichier n'est pas une video.";
            $uploadOk = 0;
        }
    }
    // Vérifier la taille du fichier
    if ($_FILES["fileToUpload_Video_Musique"]["size"] > 5000000000) {
        echo "<br>" . "Désolé, votre fichier est trop volumineux."; $uploadOk = 0;
    }
    // Autoriser certains formats de fichier
    if (($VideoFileType == "mp4") || ($VideoFileType == "vlc") ) {
        echo "<br>" . "Fichier autorisé. Format  = MP4 | VLC.";
        $uploadOk = 1;
    } else {
        echo "<br>" . "Désolé. Seuls fichiers en format MP4, VLC sont autorisés.";
        $uploadOk = 0;
    }
    // Vérifiez si $uploadOk est défini comme 0 par une erreur
    if ($uploadOk == 0) {
        echo "<br>" . "Désolé, votre fichier video n'a pas été téléchargé.";
        // si tout est correct, télécharger le fichier
    } else {
        if (move_uploaded_file($_FILES["fileToUpload_Video_Musique"]["tmp_name"], $target_file)) {
            echo "<br>" . "Le fichier ". basename( $_FILES["fileToUpload_Video_Musique"]["name"]). " a été
    téléchargé.";
        } else {
            echo "<br>" . "Désolé, une erreur s'est produite lors de l'envoi de votre
    fichier video."; }
    ...
    $VideoM= basename( $_FILES["fileToUpload_Video_Musique"]["name"]);
...
if($type === "SportsEtLoisirs") {
    ...
    /////////////Pour la Video SportsEtLoisirs
    //"Images/Video/" = subdirectory for images in www directory
    $target_dir = "Images/Video/";
    $target_file = $target_dir . basename($_FILES["fileToUpload_Video_SeL"]["name"]);
    $uploadOk = 1;
    //file extension in lower case
    $VideoFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Vérifier si le fichier image est une image réelle ou une image fausse
    if(isset($_POST["SignIn"])) {
        $check = getimagesize($_FILES["fileToUpload_Video_SeL"]["tmp_name"]);
        if($check !== false) {
            echo "Le fichier est une image - " . $check["mime"] . ".";
            $uploadOk = 1;
        } else {
    ...
    // Vérifier la taille du fichier
    if ($_FILES["fileToUpload_Video_SeL"]["size"] > 5000000000) {
        echo "<br>" . "Désolé, votre fichier est trop volumineux."; $uploadOk = 0;
    }
    // Autoriser certains formats de fichier
    if (($VideoFileType == "mp4") || ($VideoFileType == "vlc") ) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload_Video_SeL"]["tmp_name"], $target_file)) {
            echo "<br>" . "Le fichier ". basename( $_FILES["fileToUpload_Video_SeL"]["name"]). " a été
    téléchargé.";
        } else {
            echo "<br>" . "Désolé, une erreur s'est produite lors de l'envoi de votre
    fichier."; }
    ...
    $VideoSeL = basename( $_FILES["fileToUpload_Video_SeL"]["name"]);
...
if($type === "Livres") {
    ...
    //"Images/Video/" = subdirectory for images in www directory
    $target_dir = "Images/Video/";
    $target_file = $target_dir . basename($_FILES["fileToUpload_Video_Livre"]["name"]);
    $uploadOk = 1;
    //file extension in lower case
    $VideoFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Vérifier si le fichier image est une image réelle ou une image fausse
    if(isset($_POST["SignIn"])) {
        $check = getimagesize($_FILES["fileToUpload_Video_Livre"]["tmp_name"]);
        if($check !== false) {
            echo "Le fichier est une image - " . $check["mime"] . ".";
            $uploadOk = 1;
        } else {
    ...
    // Vérifier la taille du fichier
    if ($_FILES["fileToUpload_Video_Livre"]["size"] > 5000000000) {
        echo "<br>" . "Désolé, votre fichier est trop volumineux."; $uploadOk = 0;
    }
    // Autoriser certains formats de fichier
    if (($VideoFileType == "mp4") || ($VideoFileType == "vlc") ) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload_Video_Livre"]["tmp_name"], $target_file)) {
            echo "<br>" . "Le fichier ". basename( $_FILES["fileToUpload_Video_Livre"]["name"]). " a été
    téléchargé.";
        } else {
            echo "<br>" . "Désolé, une erreur s'est produite lors de l'envoi de votre
    fichier."; }
    ...
    $VideoL= basename( $_FILES["fileToUpload_Video_Livre"]["name"]);
    ...
    if($db_found) {
        ...
        $sql1= "INSERT INTO Livres (`Id`, `IdVendeur`, `Nom`, `Prix`, `Description`, `NombrePage`, `Auteur`, `Editeur`, `QuantiteVendue`, `Quantite`, `Image`, `Video`)
        values ('$id', '$id_vendeur', '$NomL', '$PrixL', '$DescriptionL', '$Nb_PagesL', '$AuteurL', '$EditeurL', '0', '$QuantiteL', '$PhotoL', '$VideoL')";
...
if($type === "Vetements") {
    ...
    /////////////Pour la Video Vetement
    //"Images/Video/" = subdirectory for images in www directory
    $target_dir = "Images/Video/";
    $target_file = $target_dir . basename($_FILES["fileToUpload_Video_Vetement"]["name"]);
    $uploadOk = 1;
    //file extension in lower case
    $VideoFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Vérifier si le fichier image est une image réelle ou une image fausse
    if(isset($_POST["SignIn"])) {
        $check = getimagesize($_FILES["fileToUpload_Video_Vetement"]["tmp_name"]);
        if($check !== false) {
            echo "Le fichier est une image - " . $check["mime"] . ".";
            $uploadOk = 1;
        } else {
    ...
    // Vérifier la taille du fichier
    if ($_FILES["fileToUpload_Video_Vetement"]["size"] > 5000000000) {
        echo "<br>" . "Désolé, votre fichier est trop volumineux."; $uploadOk = 0;
    }
    // Autoriser certains formats de fichier
    if (($VideoFileType == "mp4") || ($VideoFileType == "vlc") ) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload_Video_Vetement"]["tmp_name"], $target_file)) {
            echo "<br>" . "Le fichier ". basename( $_FILES["fileToUpload_Video_Vetement"]["name"]). " a été
    téléchargé.";
        } else {
            echo "<br>" . "Désolé, une erreur s'est produite lors de l'envoi de votre
    fichier."; }
    ...
    $PhotoVet= basename( $_FILES["fileToUpload_Vetement"]["name"]);
    $VideoVet= basename( $_FILES["fileToUpload_Video_Vetement"]["name"]);
    ...
    if($db_found) {
        ...
        $sql1= "INSERT INTO Vetements (`Id`, `IdVendeur`, `Nom`, `Prix`, `Description`, `Couleur`, `Marque`, `Taille`, `QuantiteVendue`, `Quantite`, `Image`, `Video`)
        values ('$id', '$id_vendeur', '$NomVet', '$PrixVet', '$DescriptionVet', '$CouleurVet', '$MarqueVet', '$TailleVet', '0', '$QuantiteVet', '$PhotoVet', '$VideoVet')";
